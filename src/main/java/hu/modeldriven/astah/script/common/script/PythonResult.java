...
import hu.modeldriven.astah.script.common.logging.Logger;
...
import hu.modeldriven.astah.script.common.result.TabularResultPropertyReader;
...
public class PythonResult {
    ...
    public Object asJava() {
        ...
        Optional<Object> p2j = pythonBaseTypeToJava(pyObject);
    ...
    private Object pythonObjectDerivedToJava(PyObjectDerived derived) {
        Object pythonResult = derived.__tojava__(Object.class);
        ...
        if (pythonResult instanceof TabularResult) {
            ...
            TabularResult tabularResult = (TabularResult) pythonResult;
    ...
    private List<Object> tabularResultToJava(TabularResult result) {
        ...
        for (Object entry : result.getList()) {
            if (entry instanceof PyInstance) {
                list.add(convertPythonObjectToMap((PyInstance) entry, result.getFields()));
            } else if (entry instanceof INamedElement) {
                list.add(convertAstahObjectToMap(entry, result.getFields()));
            } else {
    ...
    private Object convertAstahObjectToMap(Object instance, Map<String, String> fields) {
        ...
        Logger.log("Astah Object converted to map = " + properties);
    ...
    private Object convertPythonObjectToMap(PyInstance instance, Map<String, String> fields) {
        ...
        for (String key : fields.keySet()) {
            PyObject pyObject = instance.__findattr_ex__(key);
